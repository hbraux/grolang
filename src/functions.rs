...
pub fn load_functions(scope: &mut Scope) {
    ...
    scope.add(Fun("Int.add".to_owned(), spec(), Lambda(|args| Ok(Int(args[0].int()? + args[1].int()?)))));
    scope.add(Fun("Int.sub".to_owned(), spec(), Lambda(|args| Ok(Int(args[0].int()? - args[1].int()?)))));
    scope.add(Fun("Int.mul".to_owned(), spec(), Lambda(|args| Ok(Int(args[0].int()? * args[1].int()?)))));
    scope.add(Fun("Int.div".to_owned(), spec(), Lambda(|args| divide_int(args[0].int()?, args[1].int()?))));
    scope.add(Fun("Int.mod".to_owned(), spec(), Lambda(|args| modulo_int(args[0].int()?, args[1].int()?))));

    // float arithmetics
    let spec = || Type::new("(Float,Float)->Float");
    scope.add(Fun("Float.add".to_owned(), spec(), Lambda(|args| Ok(Float(args[0].float()? + args[1].float()?)))));
    scope.add(Fun("Float.sub".to_owned(), spec(), Lambda(|args| Ok(Float(args[0].float()? - args[1].float()?)))));
    scope.add(Fun("Float.mul".to_owned(), spec(), Lambda(|args| Ok(Float(args[0].float()? * args[1].float()?)))));
    scope.add(Fun("Float.div".to_owned(), spec(), Lambda(|args| divide_float(args[0].float()?, args[1].float()?))));
    // boolean logic
    let spec = || Type::new("(Bool,Bool)->Bool");
    scope.add(Fun("Bool.and".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].bool()? && args[1].bool()?)))));
    scope.add(Fun("Bool.or".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].bool()? || args[1].bool()?)))));
    // comparisons
    let spec = || Type::new("(Int,Int)->Bool");
    scope.add(Fun("Int.eq".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].int()? == args[1].int()?)))));
    scope.add(Fun("Int.neq".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].int()? != args[1].int()?)))));
    scope.add(Fun("Int.gt".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].int()? > args[1].int()?)))));
    scope.add(Fun("Int.ge".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].int()? >= args[1].int()?)))));
    scope.add(Fun("Int.lt".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].int()? < args[1].int()?)))));
    scope.add(Fun("Int.le".to_owned(), spec(), Lambda(|args| Ok(Bool(args[0].int()? <= args[1].int()?)))));

    //  macros
    scope.add(Fun("var".to_owned(), MutatingFun, Mutating(|args, scope| declare(args[0].symbol()?, args[1].to_type()?, args[2].eval(scope)?, scope, true))));
    scope.add(Fun("val".to_owned(), MutatingFun, Mutating(|args, scope| declare(args[0].symbol()?, args[1].to_type()?, args[2].eval(scope)?, scope, false))));
    scope.add(Fun("fun".to_owned(), MutatingFun, Mutating(|args, scope| define(args[0].symbol()?, args[1].to_params()?, args[2].to_type()?, &args[3], scope))));
    scope.add(Fun("assign".to_owned(), MutatingFun, Mutating(|args, scope| assign(args[0].symbol()?, args[1].eval(scope)?, scope))));

    scope.add(Fun("block".to_owned(), LazyFun, Lazy(|args, scope| block(args, scope))));
    scope.add(Fun("print".to_owned(), LazyFun, Lazy(|args, scope| print(args, scope))));
    scope.add(Fun("while".to_owned(), LazyFun, Lazy(|args, scope| run_while(args, scope))));
    scope.add(Fun("if".to_owned(), LazyFun, Lazy(|args, scope| if_else!(args[0].eval(scope)?.bool()? => args[1].eval(scope) ; args[2].eval(scope)))));
}
...
fn define(name: &str, params: &Vec<(String, Type)>, output: &Type, expr: &Expr, scope: &mut Scope) -> Result<Expr, Exception> {
    if scope.is_defined(&name) {
        ...
        scope.add(Fun(name.to_owned(), types, UserDefined(params.iter().map(|p| p.0.clone()).collect(), Box::new(expr.clone()))));
        Ok(Symbol(name.to_owned()))
    }
}
